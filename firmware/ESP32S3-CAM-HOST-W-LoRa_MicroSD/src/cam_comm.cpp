...
void handle_cmd(void) {
  if (recv_cmd_flag) {     // command is correct length
    ...
    if (recv_cmd_decoded[2] == TURN_ON_CAM_CODE) { // turn on the camera
    ...
    else if (recv_cmd_decoded[2] == GRAB_DATA_CAM_CODE) { // send photo data
      ...
      // send reply after data is saved to SD card
      size_t ack_len = pack_ack(SD_DONE_CODE);
      send_reply(reply_ack_encoded, ack_len);      // Send acknowledgment reply
      ...
      //   seq_num++;
      //   delay(1); // Small delay to ensure the receiver can process the packets
      // }
